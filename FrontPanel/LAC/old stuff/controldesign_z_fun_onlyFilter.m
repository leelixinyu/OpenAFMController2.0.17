...
function [K,Ld,status] = controldesign_z_fun_onlyFilter(G_freqresp, Ts, fc, order_x, order_y)

s = tf('s');
z = tf('z',Ts);

i = find(G_freqresp.Frequency > 1e3*2*pi,1);
...
tmp = abs(freqresp( tf(b,a,Ts) ,wobj));
tmp(:,:,end-3:end) = tmp(:,:,end-4);
for i=1:length(wobj)
    Wol(:,:,i) = 1.2*inv( tmp(i)*diag(ones(1,n)) );
end
Wol = frd(Wol,wobj);
...
% clear W3
% wc = 1.2*fc*2*pi; %0.9*pi/Ts;
% [b,a] = butter(2, wc/(pi/Ts));
% tmp = freqresp( tf(1/freqresp(G{1},wmin)), wobj);
% tmp = abs(tmp(:));
% tmp(end) = tmp(end-1); %last value would be 0 and cant be inverted
% for i=1:length(wobj)
%     if wobj(i) < fc*2*pi
...
%     Constraints = [Constraints, sum(X) == sum(Y)];
    ...
    for jj=1:nmod
        for j = 1:length(wobj) % Loop for constraints at each frequency point
            ...
            Constraints = [Constraints, [ctranspose(Yk_stab)*Yk_c_stab+ctranspose(Yk_c_stab)*Yk_stab-tmp] >= 0 ];   
            ...
            Constraints = [Constraints, [ctranspose(Pk)*Pk_c + ctranspose(Pk_c)*Pk - tmp]/scalem >= 0 ];
            
            %Open-loop TF constraint
%             if wobj(j) >= 0.8*fc*2*pi
                Wolk = Wol_w(:,:,j);
