function [K,Ld,status] = controldesign_x_prbs(G_freqresp_x, Ts, fc)
% clear
% 
% load G_frd_x
% fc = 1e3;
...
G{1} = G_freqresp_x / abs(freqresp(G_freqresp_x,wobj(1)));
G{1}.Ts=Ts;
...
[b,a] = butter(2, 2*fc*2*Ts);
WL = abs(freqresp(inv(tf(b,a,Ts)),wobj));
WL(end-5:end) = WL(end-6);
WL = frd(WL,wobj);
...
%                   Constraints = [Constraints, [ctranspose(Yk)*Yk_c+ctranspose(Yk_c)*Yk-tmp, ctranspose(WLk*Gk*Xk); ...
%                                          WLk*Gk*Xk, 1] >=0 ];
            end
