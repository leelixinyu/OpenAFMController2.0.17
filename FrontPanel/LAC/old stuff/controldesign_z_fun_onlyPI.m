...
function [K,Ld,status] = controldesign_z_fun_onlyPI(G_freqresp, Ts, fc)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nx = 1; ny = 0; %Controller Order

for j=1:n
    Y_c(j,j,:) = [zeros(1,ny) 1];
    X_c(j,j,:) = [zeros(1,nx-1) dcgain(Kinit) 0];
end
...
clear W3
tmp = freqresp( tf(abs(1/freqresp(G{1},wmin))), wobj);
tmp = abs(tmp(:));
tmp(end) = tmp(end-1); %last value would be 0 and cant be inverted
for i=1:length(wobj)
    if wobj(i) < fc*2*pi
...
%                             W1k*Yk/sqrt(scalem), eye(n)] >=  0) ]; 
    ...
    if(max(abs(zero(Xstab))) > 1 )
       display('Warning - Unstable Zero appeared in Controller!')
        break;
    end        
