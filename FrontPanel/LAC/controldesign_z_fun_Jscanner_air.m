% Good orders: order_x = 9, order_y = 8

function [K,Ld,status] = controldesign_z_fun_Jscanner_air(G_freqresp, Ts, fc, order_x, order_y)
...
[b,a] = butter(2, wc/(pi/Ts));
tmp = freqresp( 1/freqresp(G{1},wmin)*tf(b,a,Ts), wobj);
tmp = abs(tmp(:));
tmp(end) = tmp(end-1); %last value would be 0 and cant be inverted
for i=1:length(wobj)
    if wobj(i) < fc*2*pi
